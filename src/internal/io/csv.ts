...
export function parseCsvText(text: string, options: ReadCSVOptions = {}): DataFrame {
  ...
  if (!normalizedText.includes('"')) {
    return parseUnquotedCsvText(normalizedText, sep, options, naValues);
  }

  const rows = parseCsvRows(normalizedText, sep);
...
function parseUnquotedCsvText(
  ...
  const firstDataIndex = firstNonEmptyLineIndex(lines, 0);
  if (firstDataIndex < 0) {
    return new DataFrame([]);
  }

  const header = options.header ?? true;
  const firstFields = splitFastLine(lines[firstDataIndex]!, sep);
  let columns: string[];
  let startLineIndex: number;

  if (options.names && options.names.length > 0) {
    columns = [...options.names];
    startLineIndex = header ? firstDataIndex + 1 : firstDataIndex;
  } else if (header) {
    columns = firstFields.map((value) => value.trim());
    startLineIndex = firstDataIndex + 1;
  } else {
    columns = firstFields.map((_, position) => `col_${position}`);
    startLineIndex = firstDataIndex;
  }
  ...
  for (let i = startLineIndex; i < lines.length; i += 1) {
    ...
    const rawRow = splitFastLine(line, sep);
    while (rawRow.length > columns.length) {
      columns.push(`col_${columns.length}`);
      const newColumn = columns.at(-1)!;
      for (const existing of records) {
        existing[newColumn] = null;
      }
    }
...
function firstNonEmptyLineIndex(lines: string[], start = 0): number {
...
function splitFastLine(line: string, sep: string): string[] {
